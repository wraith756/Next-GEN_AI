...
def playAssistantSound():
    try:
        ...
        music_path = os.path.join(current_dir, "..", "www", "assets", "audio", "start_sound.mp3")
...
def closeCommand(query):
    """Close any open application window by name"""
    query = query.replace(ASSISTANT_NAME, "")
    query = query.replace("close", "").replace("exit", "").replace("quit", "")
    app_name = query.strip().lower()

    # Try closing by window title
    try:
        windows = gw.getAllTitles()
        matched = [w for w in windows if app_name in w.lower() and w.strip() != ""]
        if matched:
            for w in matched:
